...
import com.breens.foodadminapp.feature_tasks.state.ChatScreenUiState
import com.breens.foodadminapp.feature_tasks.state.SignInScreenUiState
import com.breens.foodadminapp.theme.BlueMess
import com.breens.foodadminapp.theme.LightRed
import com.breens.foodadminapp.theme.LightYellow
            ...
            Box(
                ...
                LazyColumn(
                    ...
                    items(uiState.messages.size) { index->
                        nameUser = uiState.messages[index].senderID
                        Column(
                            modifier = Modifier.fillMaxWidth(),
                            horizontalAlignment = if (uiState.messages[index].direction) Alignment.End else Alignment.Start
                        ) {
                            Box(
                                modifier = Modifier
                                    .background(
                                        if (uiState.messages[index].direction) LightYellow else LightRed,
                                        RoundedCornerShape(100.dp)
                                    ),
                                contentAlignment = Center
                            ) {
                                Text(
                                    text = uiState.messages[index].message, style = TextStyle(
                                        color = Color.Black,
                                        fontSize = 15.sp
                                    ),
                                    modifier = Modifier.padding(
                            ...
                            Text(
                                text = uiState.messages[index].createdAt,
                                style = TextStyle(
                                    color = com.breens.foodadminapp.theme.Gray,
                                    fontSize = 12.sp
                                ),
        ...
        Column() {
            ...
            TextField(
                ...
                trailingIcon = {
                    Box(
                        modifier = Modifier
                            .background(com.breens.foodadminapp.theme.Yellow, CircleShape)
